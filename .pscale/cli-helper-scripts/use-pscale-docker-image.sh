...
function pscale {
    ...
    if [ -n "$NO_DOCKER" ]; then
        ...
        command="docker run -e PLANETSCALE_SERVICE_TOKEN=${PLANETSCALE_SERVICE_TOKEN:-""} -e PLANETSCALE_SERVICE_TOKEN_ID=$PLANETSCALE_SERVICE_TOKEN_ID -e PLANETSCALE_SERVICE_TOKEN_NAME=$PLANETSCALE_SERVICE_TOKEN_NAME -e HOME=/tmp -v $HOME/.config/planetscale:/tmp/.config/planetscale -v $DIR:$DIR -e PSCALE_ALLOW_NONINTERACTIVE_SHELL=true --rm -i $tty planetscale/pscale:${PSCALE_VERSION:-"latest"} $@"
    fi
